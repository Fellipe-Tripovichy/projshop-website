...
export default function SmallCardCarousel({slides} : Props) {
  ...
  return (
    <div className="block md:hidden overflow-hidden relative">
      ...
      >
        {slides.map((item, idx) => {
          return <SmallProductCard imageList={item.imageList} key={idx} tagList={item.tagList} title={item.title} descriptionList={item.descriptionList}/>;
        })}
      </div>
